#!/usr/bin/env bash
# Tokenize + split FineWeb-2 Thai (fineweb2_tha, byte_premium=2.74, eval_lines=21920)
...
TOK_DIR="${OUTPUT_ROOT}/bpe_tha_thai_8192_300mb_unscaled"

python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
  --dataset fineweb2_tha --tokenizer_type bpe_unscaled --tokenizer_vocabulary 8192 \
...
# mv "${TOK_DIR}/fineweb2_tha_2.74_tokenized.txt" \
#    "${TOK_DIR}/fineweb2_tha_2.74_tokenized_full.txt"
#
...
# ENG_BYTES=$(stat -c%s "$ENG_REF")
# TARGET_BYTES=$(python3 -c "import math; print(math.floor(2.74 * $ENG_BYTES))")
# TOK_DIR="${OUTPUT_ROOT}/bpe_tha_thai_32768_300mb_unscaled"
#
# python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
...
# ENG_BYTES=$(stat -c%s "$ENG_REF")
# TARGET_BYTES=$(python3 -c "import math; print(math.floor(2.74 * $ENG_BYTES))")
# TOK_DIR="${OUTPUT_ROOT}/bpe_tha_thai_65536_300mb_unscaled"
#
# python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
#   --dataset fineweb2_tha --tokenizer_type bpe_unscaled --tokenizer_vocabulary 65536 \
#   --split train --max_seq_len 1024 --max_segments -1 \
#   --prepend_cls True --include_sep True --shuffle False \
#   --index_path "$INDEX_PATH" --output_root "$OUTPUT_ROOT"
#
...
ENG_BYTES=$(stat -c%s "$ENG_REF")
TARGET_BYTES=$(python3 -c "import math; print(math.floor(2.74 * $ENG_BYTES))")
TOK_DIR="${OUTPUT_ROOT}/bpe_tha_thai_98304_300mb_unscaled"

python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
  --dataset fineweb2_tha --tokenizer_type bpe_unscaled --tokenizer_vocabulary 98304 \
  --split train --max_seq_len 1024 --max_segments -1 \
  --prepend_cls True --include_sep True --shuffle False \
  --index_path "$INDEX_PATH" --output_root "$OUTPUT_ROOT"
...
ENG_BYTES=$(stat -c%s "$ENG_REF")
TARGET_BYTES=$(python3 -c "import math; print(math.floor(2.74 * $ENG_BYTES))")
TOK_DIR="${OUTPUT_ROOT}/unigram_tha_thai_8192_300mb_unscaled"

python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
  --dataset fineweb2_tha --tokenizer_type unigram_unscaled --tokenizer_vocabulary 8192 \
  --split train --max_seq_len 1024 --max_segments -1 \
  --prepend_cls True --include_sep True --shuffle False \
  --index_path "$INDEX_PATH" --output_root "$OUTPUT_ROOT"
...
# ENG_BYTES=$(stat -c%s "$ENG_REF")
# TARGET_BYTES=$(python3 -c "import math; print(math.floor(2.74 * $ENG_BYTES))")
# TOK_DIR="${OUTPUT_ROOT}/unigram_tha_thai_32768_300mb_unscaled"
#
# python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
#   --dataset fineweb2_tha --tokenizer_type unigram_unscaled --tokenizer_vocabulary 32768 \
#   --split train --max_seq_len 1024 --max_segments -1 \
#   --prepend_cls True --include_sep True --shuffle False \
#   --index_path "$INDEX_PATH" --output_root "$OUTPUT_ROOT"
#
...
# ENG_BYTES=$(stat -c%s "$ENG_REF")
# TARGET_BYTES=$(python3 -c "import math; print(math.floor(2.74 * $ENG_BYTES))")
# TOK_DIR="${OUTPUT_ROOT}/unigram_tha_thai_65536_300mb_unscaled"
#
# python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
#   --dataset fineweb2_tha --tokenizer_type unigram_unscaled --tokenizer_vocabulary 65536 \
#   --split train --max_seq_len 1024 --max_segments -1 \
#   --prepend_cls True --include_sep True --shuffle False \
#   --index_path "$INDEX_PATH" --output_root "$OUTPUT_ROOT"
#
...
ENG_BYTES=$(stat -c%s "$ENG_REF")
TARGET_BYTES=$(python3 -c "import math; print(math.floor(2.74 * $ENG_BYTES))")
TOK_DIR="${OUTPUT_ROOT}/unigram_tha_thai_98304_300mb_unscaled"

python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
  --dataset fineweb2_tha --tokenizer_type unigram_unscaled --tokenizer_vocabulary 98304 \
  --split train --max_seq_len 1024 --max_segments -1 \
  --prepend_cls True --include_sep True --shuffle False \
  --index_path "$INDEX_PATH" --output_root "$OUTPUT_ROOT"
