#!/bin/bash
#
# Width scaling experiment for CompleteP with Learned Position Embeddings only
# (RMSNorm + SwiGLU + Learned)
#
# This isolates the effect of Learned vs RoPE position embeddings on activation drift.
...
BASE_DIR="coord_check_experiments/width_scaling/completep_learned_pos"
...
echo "Running CompleteP width scaling with Learned Position Embeddings only..."
echo "  Norm: RMSNorm (LLAMA standard)"
echo "  MLP: SwiGLU (LLAMA standard)"
echo "  Position: Learned"
echo ""
...
echo ""
echo "Width scaling experiment complete (Learned Position Embeddings only)!"
