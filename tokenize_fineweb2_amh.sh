#!/usr/bin/env bash
# Tokenize + split FineWeb-2 Amharic (fineweb2_amh, byte_premium=1.72, eval_lines=13760)
...
TARGET_BYTES=$(python3 -c "import math; print(math.floor(1.72 * $ENG_BYTES))")
TOK_DIR="${OUTPUT_ROOT}/bpe_amh_ethi_8192_300mb_unscaled"

python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
  --dataset fineweb2_amh --tokenizer_type bpe_unscaled --tokenizer_vocabulary 8192 \
...
# ENG_BYTES=$(stat -c%s "$ENG_REF")
# TARGET_BYTES=$(python3 -c "import math; print(math.floor(1.72 * $ENG_BYTES))")
# TOK_DIR="${OUTPUT_ROOT}/bpe_amh_ethi_32768_300mb_unscaled"
#
# python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
#   --dataset fineweb2_amh --tokenizer_type bpe_unscaled --tokenizer_vocabulary 32768 \
#   --split train --max_seq_len 1024 --max_segments -1 \
#   --prepend_cls True --include_sep True --shuffle False \
#   --index_path "$INDEX_PATH" --output_root "$OUTPUT_ROOT"
#
...
# ENG_BYTES=$(stat -c%s "$ENG_REF")
# TARGET_BYTES=$(python3 -c "import math; print(math.floor(1.72 * $ENG_BYTES))")
# TOK_DIR="${OUTPUT_ROOT}/bpe_amh_ethi_65536_300mb_unscaled"
#
# python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
#   --dataset fineweb2_amh --tokenizer_type bpe_unscaled --tokenizer_vocabulary 65536 \
#   --split train --max_seq_len 1024 --max_segments -1 \
#   --prepend_cls True --include_sep True --shuffle False \
#   --index_path "$INDEX_PATH" --output_root "$OUTPUT_ROOT"
#
...
ENG_BYTES=$(stat -c%s "$ENG_REF")
TARGET_BYTES=$(python3 -c "import math; print(math.floor(1.72 * $ENG_BYTES))")
TOK_DIR="${OUTPUT_ROOT}/bpe_amh_ethi_98304_300mb_unscaled"

python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
  --dataset fineweb2_amh --tokenizer_type bpe_unscaled --tokenizer_vocabulary 98304 \
  --split train --max_seq_len 1024 --max_segments -1 \
  --prepend_cls True --include_sep True --shuffle False \
  --index_path "$INDEX_PATH" --output_root "$OUTPUT_ROOT"
...
ENG_BYTES=$(stat -c%s "$ENG_REF")
TARGET_BYTES=$(python3 -c "import math; print(math.floor(1.72 * $ENG_BYTES))")
TOK_DIR="${OUTPUT_ROOT}/unigram_amh_ethi_8192_300mb_unscaled"

python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
  --dataset fineweb2_amh --tokenizer_type unigram_unscaled --tokenizer_vocabulary 8192 \
  --split train --max_seq_len 1024 --max_segments -1 \
  --prepend_cls True --include_sep True --shuffle False \
  --index_path "$INDEX_PATH" --output_root "$OUTPUT_ROOT"
...
# ENG_BYTES=$(stat -c%s "$ENG_REF")
# TARGET_BYTES=$(python3 -c "import math; print(math.floor(1.72 * $ENG_BYTES))")
# TOK_DIR="${OUTPUT_ROOT}/unigram_amh_ethi_32768_300mb_unscaled"
#
# python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
#   --dataset fineweb2_amh --tokenizer_type unigram_unscaled --tokenizer_vocabulary 32768 \
#   --split train --max_seq_len 1024 --max_segments -1 \
#   --prepend_cls True --include_sep True --shuffle False \
#   --index_path "$INDEX_PATH" --output_root "$OUTPUT_ROOT"
#
...
# ENG_BYTES=$(stat -c%s "$ENG_REF")
# TARGET_BYTES=$(python3 -c "import math; print(math.floor(1.72 * $ENG_BYTES))")
# TOK_DIR="${OUTPUT_ROOT}/unigram_amh_ethi_65536_300mb_unscaled"
#
# python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
#   --dataset fineweb2_amh --tokenizer_type unigram_unscaled --tokenizer_vocabulary 65536 \
#   --split train --max_seq_len 1024 --max_segments -1 \
#   --prepend_cls True --include_sep True --shuffle False \
#   --index_path "$INDEX_PATH" --output_root "$OUTPUT_ROOT"
#
...
ENG_BYTES=$(stat -c%s "$ENG_REF")
TARGET_BYTES=$(python3 -c "import math; print(math.floor(1.72 * $ENG_BYTES))")
TOK_DIR="${OUTPUT_ROOT}/unigram_amh_ethi_98304_300mb_unscaled"

python "${SCRIPT_DIR}/scripts/tokenize_and_pack.py" \
  --dataset fineweb2_amh --tokenizer_type unigram_unscaled --tokenizer_vocabulary 98304 \
  --split train --max_seq_len 1024 --max_segments -1 \
  --prepend_cls True --include_sep True --shuffle False \
  --index_path "$INDEX_PATH" --output_root "$OUTPUT_ROOT"
