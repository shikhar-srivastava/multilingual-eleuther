...
class LlamaLearnedPositionalEmbedding(nn.Module):
...
class LlamaSinusoidalPositionalEmbedding(nn.Module):
    ...
    def forward(self, hidden_states, position_ids=None):
        ...
        if position_ids is None:
            position_embeddings = self.pe[:, :seq_len, :].expand(batch_size, -1, -1)
        else:
            # position_ids shape: (batch_size, seq_len)
            # We need to gather the appropriate position embeddings
            position_embeddings = self.pe.expand(batch_size, -1, -1).gather(
                1, position_ids.unsqueeze(-1).expand(-1, -1, self.hidden_size)
            )
        return hidden_states + position_embeddings
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, scale_attn_weights: bool = False):
        ...
        # Initialize positional embedding based on type
        if self.position_embedding_type == 'rope':
            self.rotary_emb = LlamaRotaryEmbedding(self.head_dim, max_position_embeddings=self.max_position_embeddings)
        elif self.position_embedding_type == 'learned':
            # For learned positional embeddings, we don't need rotary embedding
            self.rotary_emb = None
        elif self.position_embedding_type == 'sinusoidal':
            # For sinusoidal positional embeddings, we don't need rotary embedding
            self.rotary_emb = None
    ...
    def forward(
        ...
        if self.position_embedding_type == 'rope' and self.rotary_emb is not None:
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        # Initialize positional embeddings for learned and sinusoidal types
        if self.position_embedding_type == 'learned':
            self.position_embeddings = LlamaLearnedPositionalEmbedding(
                self.max_position_embeddings, config.hidden_size
            )
        elif self.position_embedding_type == 'sinusoidal':
            self.position_embeddings = LlamaSinusoidalPositionalEmbedding(
                self.max_position_embeddings, config.hidden_size
            )
        else:
    ...
    def forward(
        ...
        # Apply positional embeddings for learned and sinusoidal types
