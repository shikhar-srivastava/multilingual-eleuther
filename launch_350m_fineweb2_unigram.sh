...
# NOTE: Uses BPE English refs (the larger of the two tokenizer types) so that
# the raw file is sufficient regardless of which launch script runs first.
...
if [[ ! -f "$WORST_ENG_REF" ]]; then
  # Fall back to unigram ref if BPE hasn't been run yet
  DOWNLOAD_PREFIX="unigram"
  WORST_ENG_REF="${OUTPUT_ROOT}/${DOWNLOAD_PREFIX}_eng_latn_${WORST_VOCAB}_300mb_unscaled/fineweb_eng_1.0_tokenized.txt"
fi
if [[ ! -f "$WORST_ENG_REF" ]]; then
  echo "[ERROR] English reference not found for either BPE or Unigram vocab=${WORST_VOCAB}"
